...
const _getLastMessage = async ({ channel, uid }) => {
  if (channel) {
    const ref = firebase
      .database()
      .ref(`messages/${channel}/users/${Fire.shared.uid}`);
    ...
    __message.name = await ProfileProvider.getPropertyForUser({
      uid,
      property: 'first_name',
    });
...
const uploadMessageData = async (
  ...
  if (image) {
    ...
    const res = await ImageProvider.uploadAsset({
      path: photourl,
      image,
      onProgress,
    });
...
export const deleteChannel = (channel) => {
  ...
  Alert.alert(
    ...
    [
      ...
      {
        text: Meta.delete_message_channel_destructive,
